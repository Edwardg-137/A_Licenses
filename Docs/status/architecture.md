...
**Implementado (Fases 2–5):** `BORRADOR → (OBSERVADO_FORMATO) → EN_REVISION_TECNICA ⇄ EN_CORRECCION → ALINEACION_PROGRAMADA → PENDIENTE_DE_PAGO → LICENCIA_EMITIDA → RECEPCION_DE_OBRA → CERRADO`, y la salida a `RECHAZADO` (con dictamen). La validación automática es síncrona al enviar (por eso `EN_VALIDACION` no aparece como estado persistido: dura segundos). Responsables de transición: `applications` (ingreso), `review` (revisión y correcciones, D-010/D-011), `inspections` (alineación y recepción de obra), `payments` (cálculo F08 y confirmación — D-012) y `licenses` (emisión automática del PDF + QR al confirmar el pago — D-014). Toda transición registra auditoría y notificación. La verificación pública de licencia es el único endpoint de negocio `@Public()` además de auth.
