...
Tipos en línea para la Municipalidad de Guatemala:

- **L-01** Obra Mayor — Vivienda Unifamiliar (**F08**, ≤ 700 m²)
- **L-02** Obra Mayor — Comercial / Mixto / General (**F02**, 31–700 m²)

Arquitectura multi-tenant preparada para más municipios. F11 (L-03) y F04 (L-06) mapeados, pendientes de implementación.
...
- **Pago de tasa municipal:** cálculo automático configurable (`base + % sobre presupuesto estimado` en `LicenseType.feeFormula`; provisionales distintos L-01/L-02); vista con desglose; **pago en línea simulado** (banner de simulación) que genera comprobante PDF adjunto como D-15, o carga de comprobante externo; confirmación por revisor/Admin (D-012) → `LICENCIA_EMITIDA` (PDF de la licencia en Fase 5).
...
- **Seed:** tenant "Municipalidad de Guatemala", **dos usuarios de prueba ACTIVE por cada rol**, tipos **L-01 (F08, D-01…D-15)** y **L-02 (F02, D-01…D-21; D-16…D-21 opcionales)**.
...
- La fórmula de la tasa usa valores provisionales en el seed (L-01 y L-02); el arancel real está pendiente de conseguirse.
- Solo L-01/F08 y L-02/F02 en línea; F11 (L-03), F04 (L-06) y demás tipos aún no.
...
| Solicitante | `solicitante@permisogt.local` | `Solicita123` | Crear expedientes F08/F02, cargar/verificar documentos, enviar a revisión; atender observaciones; confirmar fecha de visita; pagar (simulado o comprobante externo); solicitar recepción de obra |
